...
function select_image(element){
  $('.asset_list li').removeClass("selected")
    element = $(element);
...
function select_thumb(element) {
  ...
  insert_asset(element.attr('href') );
}

function upload_callback(channel){
    //$('#dynamic_images_list').html('');
    $.ajax({type: "GET", url: "/assets/manage?media=video&page=1", dataType: "script"});
}

function insert_asset(path){
...
function upload_media_path() {
  ...
  to_path = "/assets/create?media=video";
