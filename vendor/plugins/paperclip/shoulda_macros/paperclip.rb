...
module Paperclip
  ...
  module Shoulda
    include Matchers
    # This will test whether you have defined your asset correctly by
    ...
    # Tests for validations on the presence of the asset.
    def should_validate_asset_presence name
      klass   = self.name.gsub(/Test$/, '').constantize
      matcher = validate_asset_presence name
      should matcher.description do
        assert_accepts(matcher, klass)
      end
    end
    ...
    def should_validate_asset_content_type name, options = {}
      klass   = self.name.gsub(/Test$/, '').constantize
      valid   = [options[:valid]].flatten
      invalid = [options[:invalid]].flatten
      matcher = validate_asset_content_type(name).allowing(valid).rejecting(invalid)
      should matcher.description do
        assert_accepts(matcher, klass)
      end
    end
    ...
    # validate_asset_file_size - :less_than, :greater_than, and :in.
    ...
    def should_validate_asset_size name, options = {}
      klass   = self.name.gsub(/Test$/, '').constantize
      min     = options[:greater_than] || (options[:in] && options[:in].first) || 0
      max     = options[:less_than]    || (options[:in] && options[:in].last)  || (1.0/0)
      range   = (min..max)
      matcher = validate_asset_size(name).in(range)
      should matcher.description do
        assert_accepts(matcher, klass)
      end
    end

    # Stubs the HTTP PUT for an asset using S3 storage.
    #
    # @example
    #   stub_paperclip_s3('user', 'avatar', 'png')
    def stub_paperclip_s3(model, asset, extension)
      definition = model.gsub(" ", "_").classify.constantize.
                         asset_definitions[asset.to_sym]
    ...
    # Stub S3 and return a file for asset. Best with Factory Girl.
    ...
    #   end
    def paperclip_fixture(model, asset, extension)
      stub_paperclip_s3(model, asset, extension)
      base_path = File.join(File.dirname(__FILE__), "..", "..",
                            "features", "support", "paperclip")
      File.new(File.join(base_path, model, "#{asset}.#{extension}"))
    end
  end
end
