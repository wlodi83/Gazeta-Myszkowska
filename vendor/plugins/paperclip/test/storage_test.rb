...
class StorageTest < Test::Unit::TestCase
  ...
  context "An asset with S3 storage" do
    setup do
      rebuild_model :storage => :s3,
                    :bucket => "testing",
                    :path => ":asset/:style/:basename.:extension",
                    :s3_credentials => {
                      'access_key_id' => "12345",
  ...
  context "An asset with S3 storage and bucket defined as a Proc" do
    setup do
      AWS::S3::Base.stubs(:establish_connection!)
      rebuild_model :storage => :s3,
                    :bucket => lambda { |asset| "bucket_#{asset.instance.other}" },
                    :s3_credentials => {:not => :important}
    end
  ...
  context "An asset with S3 storage and specific s3 headers set" do
    setup do
      AWS::S3::Base.stubs(:establish_connection!)
      rebuild_model :storage => :s3,
                    :bucket => "testing",
  ...
  unless ENV["S3_TEST_BUCKET"].blank?
    context "Using S3 for real, an asset with S3 storage" do
      setup do
        rebuild_model :styles => { :thumb => "100x100", :square => "32x32#" },
                      :storage => :s3,
                      :bucket => ENV["S3_TEST_BUCKET"],
                      :path => ":class/:asset/:id/:style.:extension",
