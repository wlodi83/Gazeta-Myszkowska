...
class IntegrationTest < Test::Unit::TestCase
  ...
  context "An asset" do
    ...
    context "redefining its asset styles" do
      setup do
        Dummy.class_eval do
  ...
  context "A model with assets scoped under an id" do
    setup do
      rebuild_model :styles => { :large => "100x100",
                                 :medium => "50x50" },
                    :path => ":rails_root/tmp/:id/:assets/:style.:extension"
      @dummy = Dummy.new
      @file = File.new(File.join(File.dirname(__FILE__),
                                 "fixtures",
                                 "5k.png"), 'rb')
  ...
  context "A model with no asset validation" do
    setup do
      rebuild_model :styles => { :large => "300x300>",
                                 :medium => "100x100",
                                 :thumb => ["32x32#", :gif] },
                    :default_style => :medium,
                    :url => "/:asset/:class/:style/:id/:basename.:extension",
    ...
    should "have its definition return false when asked about whiny_thumbnails" do
      assert ! Dummy.asset_definitions[:avatar][:whiny_thumbnails]
    end

    context "when validates_asset_thumbnails is called" do
      setup do
        Dummy.validates_asset_thumbnails :avatar
      end

      should "have its definition return true when asked about whiny_thumbnails" do
        assert_equal true, Dummy.asset_definitions[:avatar][:whiny_thumbnails]
      end
    end

    context "redefined to have asset validations" do
      setup do
        rebuild_model :styles => { :large => "300x300>",
                                   :medium => "100x100",
                                   :thumb => ["32x32#", :gif] },
  ...
  context "A model with no convert_options setting" do
    ...
    should "have its definition return nil when asked about convert_options" do
      assert ! Dummy.asset_definitions[:avatar][:convert_options]
    end
    ...
    context "redefined to have convert_options setting" do
      ...
      should "have its definition return convert_options value when asked about convert_options" do
        assert_equal "-strip -depth 8", Dummy.asset_definitions[:avatar][:convert_options]
      end
    end
  end
  
  context "A model with a filesystem asset" do
    setup do
      rebuild_model :styles => { :large => "300x300>",
                                 :medium => "100x100",
                                 :thumb => ["32x32#", :gif] },
    ...
    should "know the difference between good files, bad files, and not files when validating" do
      Dummy.validates_asset_presence :avatar
      @d2 = Dummy.find(@dummy.id)
      @d2.avatar = @file
    ...
    context "that is assigned its file from another Paperclip asset" do
      setup do
        @dummy2 = Dummy.new
  ...
  context "A model with an assets association and a Paperclip asset" do
    setup do
      Dummy.class_eval do
        has_many :assets, :class_name => 'Dummy'
      end
  ...
  if ENV['S3_TEST_BUCKET']
    def s3_files_for asset
      [:thumb, :medium, :large, :original].inject({}) do |files, style|
        data = `curl "#{asset.url(style)}" 2>/dev/null`.chomp
    ...
    def s3_headers_for asset, style
      `curl --head "#{asset.url(style)}" 2>/dev/null`.split("\n").inject({}) do |h,head|
    ...
    context "A model with an S3 asset" do
      setup do
        rebuild_model :styles => { :large => "300x300>",
                                   :medium => "100x100",
                                   :thumb => ["32x32#", :gif] },
                      ...
                      :path => ":class/:asset/:id/:style/:basename.:extension"
        @dummy     = Dummy.new
        @file      = File.new(File.join(FIXTURES_DIR, "5k.png"), 'rb')
        @bad_file  = File.new(File.join(FIXTURES_DIR, "bad.png"), 'rb')
