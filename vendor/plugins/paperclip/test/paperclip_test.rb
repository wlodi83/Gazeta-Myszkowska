...
class PaperclipTest < Test::Unit::TestCase
  ...
  should "raise when sent #processor and the name of a class that exists but isn't a subclass of Processor" do
    assert_raises(Paperclip::PaperclipError){ Paperclip.processor(:asset) }
  end
  ...
  context "An ActiveRecord model with an 'avatar' asset" do
    setup do
    ...
    should "not error when trying to also create a 'blah' asset" do
    ...
    context "with a subclass" do
      ...
      should "be able to use the asset from the subclass" do
        assert_nothing_raised do
          @subdummy = SubDummy.create(:avatar => @file)
        end
      end

      should "be able to see the asset definition from the subclass's class" do
        assert_equal "tmp/:class/omg/:style.:extension",
                     SubDummy.asset_definitions[:avatar][:path]
      end
    ...
    context "that is valid" do
      ...
      context "then has a validation added that makes it invalid" do
        setup do
          assert @dummy.save
          Dummy.class_eval do
            validates_asset_content_type :avatar, :content_type => ["text/plain"]
          end
    ...
    context "a validation with an if guard clause" do
      setup do
        Dummy.send(:"validates_asset_presence", :avatar, :if => lambda{|i| i.foo })
        @dummy = Dummy.new
      end
    ...
    context "a validation with an unless guard clause" do
      setup do
        Dummy.send(:"validates_asset_presence", :avatar, :unless => lambda{|i| i.foo })
        @dummy = Dummy.new
      end
    ...
    def self.should_validate validation, options, valid_file, invalid_file
      context "with #{validation} validation and #{options.inspect} options" do
        setup do
          Dummy.send(:"validates_asset_#{validation}", :avatar, options)
          @dummy = Dummy.new
        end
        context "and assigning nil" do
          ...
          if validation == :presence
            should "have an error on the asset" do
              assert @dummy.errors.on(:avatar)
            end
          else
            should "not have an error on the asset" do
    ...
    context "with size validation and less_than 10240 option" do
      context "and assigned an invalid file" do
        setup do
          Dummy.send(:"validates_asset_size", :avatar, :less_than => 10240)
          @dummy = Dummy.new
