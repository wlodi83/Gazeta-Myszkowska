function cancel_image(){
  $('.asset_list').removeClass("selected")
}

function delete_asset(element){
  $('#dynamic_images_list').html("Uploading...<br /><img src='/images/tiny_mce/spinner.gif'>");
  $('.asset_list').removeClass("selected")
  //$.ajax({type: "GET", url: "/"+channel+"/"+feature+"/assets/manage?media=print&page=1", dataType: "script"});
}
...
function upload_callback(){
    ...
    $.ajax({type: "GET", url: "/assets/manage?media=print&page=1", dataType: "script"});
}
...
function ts_upload_image_path() {
  ...
  return "/assets/create?media=print";
}
